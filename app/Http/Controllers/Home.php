...
class Home extends BaseController
{
    ...
    protected function viewcontroller($data=array())
    {
        ...
        foreach($template_sidebar as $tk=>$tv){
            ...
            $has_access = true;
            if($api_group=='image' && !has_module_access($this->module_id_image,$this->module_ids,$this->is_admin)) {
                $has_access=false;
            }
            if($api_group=='audio' && !has_module_access($this->module_id_audio,$this->module_ids,$this->is_admin)) {
                $has_access=false;
            }
            if(($api_group=='text' || $api_group=='chat') && !has_module_access($this->module_id_token,$this->module_ids,$this->is_admin)) {
                $has_access=false;
            }
    ...
    protected function site_viewcontroller_ajax($data=array())
    {
        if (!isset($data['body'])) return false;
        $user_id = $parent_user_id = $user_type = "";
        $expired_date = null;
        ...
        $data['meta_image_height'] = $meta_image_height;
        // return view($data['body'], $data);
        $html = view($data['body'], $data)->render();
        return $html;
    }
    ...
    protected function get_payment_validity_data_razorpay($buyer_user_id=0,$package_id=0)
    {
        $package_data = $this->get_package($package_id,['package_name','price','validity','discount_data','product_data']);
        $package_name = $package_data->package_name ?? '';
        $discount_data = $package_data->discount_data ?? null;
