...
class Template extends Home{
    ...
    public function tools($group_slug,$template_slug,$search_id=null){
        ...
        if(!has_module_access($module_id,$this->module_ids,$this->is_admin)) return redirect(route('pricing-plan'));
    ...
    public function tools_action(){
        ...
        // return response()->json(['status' => '0','message' => __('Content generation has been suspended on the admin account due to heavy traffic. Kindly register as a user and access the content from your own account.')]);
        ...
        if(!has_module_access($module_id,$this->module_ids,$this->is_admin)) {
    ...
    {
        if(!has_module_action_access($this->module_id_template_manager,1,$this->team_access,$this->is_manager)) abort('403');
