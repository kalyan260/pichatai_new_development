...
class Subscription extends Home
{
    ...
    public function list_package_data(Request $request)
    {
        ...
        $table="packages";
        $query = DB::table($table)->where('user_id',$this->user_id)->where('deleted','0')->where('is_hide',0);
        if ($search_value != '')
        {
    ...
    public function save_package(Request $request)
    {
        ...
        if($package_type!='team'){
            if($request->id == '1') $rules['price'] = 'required';
        ...
        $query = true;
        // $validate_data['ref_country'] = json_encode($request->ref_country);
        $validate_data['other_price'] = $request->other_price;
        if(isset($request->id)) :
            if($request->id == 1 && $request->is_default != null) {
                $validate_data['is_default'] = '1';
                Package::where('id',5)->update([
                    'is_default' => '0'
                ]);
            } else {
                $validate_data['is_default'] = '0';
                Package::where('id',5)->update([
                    'is_default' => '1'
                ]);
                Package::where('id','!=',6)->update([
                    'is_default' => '0'
                ]);
            }
            DB::table("packages")->where(['id'=>$request->id,'user_id'=>$this->user_id])->update($validate_data);
        else : 
            $query = DB::table("packages")->insert($validate_data);
        endif;
    ...
    public function delete_user(Request $request)
    {
        ...
        if($success) {
            return response()->json(['error' => false,'message' => __('User Deleted Successfully')]);
        } else {
    ...
    public function affiliate_user_request_list(Request $request)
    {
        ...
        foreach ($info as $key => $value)
        {
            ...
            $value->website = '<a target="_BLANK" href="'.$value->website.'"><i class="fas fa-globe"></i></a> '.' <a target="_BLANK" class="ps-2" href="'.$value->fb_link.'"><i class="fab fa-facebook"></i></a>';
        }
