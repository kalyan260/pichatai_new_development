...
class Landing extends Home
{
    ...
    public function pricing_plan(){
        ...
        $get_pricing_list_news = $this->get_pricing_list_update($validity);
        ...
        foreach($get_pricing_list as $key=>$value){
            if(!isset($package_validity_list[$value->validity]) && $value->is_hide != '1'){
                if($value->is_free != '1'){
                    $package_validity_list[$value->validity] = convert_number_validity_phrase($value->validity);
                } else {
            ...
            if(!empty($validity) && $value->validity!=$validity  && $value->is_hide != '1'){
                if($value->is_free != '1'){
                    $get_pricing_list->forget($key);
                } else {
                    if($value->is_default != '0') {
                        $get_pricing_list->forget($key);
                    }
        ...
        $data['get_pricing_list_news'] = $get_pricing_list_news;
        $data['default_validity'] = $validity;
        $clientIP = request()->ip();
        $data['location'] = Location::get($clientIP);
        return $this->site_viewcontroller($data);
    }
    public function pricing_plan_ajax(){
        $validity = !empty(request()->validity) ? request()->validity : 30;
        $data = $this->make_view_data();
        $data['body'] = 'landing.pricing-ajax';
        $data['title'] = __('Pricing Plan');
        $get_pricing_list = $this->get_pricing_list();
        $get_pricing_list_news = $this->get_pricing_list_update($validity);
        $data['get_modules'] = $this->get_modules();
        ...
        $clientIP = request()->ip();
        $data['location'] = Location::get($clientIP);
        return $this->site_viewcontroller_ajax($data);
    }

    public function blog_index() {
        $validity = !empty(request()->validity) ? request()->validity : 30;
        $data = $this->make_view_data();
        $data['body'] = 'landing.blog.index';
        $data['title'] = __('Blogs');
        $data['get_modules'] = $this->get_modules();
        $data['format_settings'] = $this->get_payment_formatting_data();
        $data['default_validity'] = $validity;
        $data['items'] = Blog::orderBy('id', 'desc')->paginate(20);
        return $this->site_viewcontroller($data);
    }
    public function blog_details($id) {
        $validity = !empty(request()->validity) ? request()->validity : 30;
        $data = $this->make_view_data();
        $data['body'] = 'landing.blog.details';
        $data['title'] = __('Blogs');
        $data['get_modules'] = $this->get_modules();
        $data['format_settings'] = $this->get_payment_formatting_data();
        $data['default_validity'] = $validity;
    ...
    public function faq() {
        $validity = !empty(request()->validity) ? request()->validity : 30;
        $data = $this->make_view_data();
        $data['body'] = 'landing.faq';
        $data['title'] = __('Blogs');
        $data['get_modules'] = $this->get_modules();
        $data['format_settings'] = $this->get_payment_formatting_data();
        $data['default_validity'] = $validity;
        $data['items'] = Faq::orderBy('id', 'desc')->get();
        return $this->site_viewcontroller($data);
    }
    public function get_pricing_list_update($validity=null,$limit=9999)
    {
        $user_id = 1;
        if(empty($user_id)) $user_id = 1;
        $query = DB::table('packages')->where(['user_id'=>$user_id,'visible'=>'1','deleted'=>'0','package_type'=>'subscription','validity' => $validity,'is_hide' => 0]);
    ...
    public function policy_static($url){
        $data = $this->make_view_data();
        $data['body'] = 'landing.policy.policy-static';
        $data['item'] = Policy::where('url',$url)->first();
        $data['title'] = __('Privacy Policy');
        return $this->site_viewcontroller($data);
    }
    ...
    public function contact_us(){
        $data = $this->make_view_data(); 
        $data['body'] = 'landing.policy.contact-us';
