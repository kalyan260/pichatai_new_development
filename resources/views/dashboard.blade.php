                    ...
                    @if (!auth()->user()->email_verified_at && !$is_manager)
                        <div class="alert alert-warning alert-dismissible fade show p-4" role="alert">
                            <h5 class="alert-heading mb-1 pt-2 text-dark">
                                ...
                                {{ __('Verify Email') }} :
                                <small>{{ __('Email is not verified yet. Please verify your email.') }}</small>
                            </h5>
                            <p class="ps-sm-5 ms-sm-1 ms-lg-2"><span
                                    class="fw-bold text-dark">{{ __('Click the link to get started') }}</span> : <a
                                    href="{{ route('verification.notice') }}"
                                    class="text-success fw-bold text-decoration-none">{{ __('Start Email Verification') }}</a></span>
                    ...
                    <div class="tab-content tab-content-basic pt-0">
                        <div class="tab-pane fade show active" id="overview" role="tabpanel" aria-labelledby="overview">
                            <?php
                                ...
                                $image_limit = $usage_stat['monthly_limit'][$module_id_image] ?? 0;
                                $audio_used = $usage_stat['usage_info'][$module_id_audio]['usage_count'] ?? 0;
                                $audio_limit = $usage_stat['monthly_limit'][$module_id_audio] ?? 0;
                            ?>
                            <div class="row">
                                <div class="col-sm-12">
                                    <div class="statistics-details d-flex align-items-center justify-content-between">
                                        ...
                                        <div class="">
                                            <p class="statistics-title text-center">{{ __('Token Limit') }}</p>
                                            <h3 class="rate-percentage text-center">{!! $token_module_limit !!}</h3>
                                        </div>
                                        <div class="d-none d-md-block">
                                            <p class="statistics-title text-center">{{ __('Token Usage') }} %</p>
                                            <h3 class="rate-percentage text-center">{{ number_format($token_usage, 2) }}%
                                            </h3>
                                        </div>
                                        <div class="">
                                            <p class="statistics-title text-center d-none d-xl-block">
                                                {{ __('Speech to text') }}</p>
                                            <p class="statistics-title text-center d-xl-none">{{ __('Speech') }}</p>
                                            <h3 class="rate-percentage text-center">{{ $audio_used }} {{ __('mn') }} / {{ $audio_limit }} {{ __('min') }}</h3>
                                        </div>
                                        <div class="">
                                            <p class="statistics-title text-center d-none d-xl-block">
                                                {{ __('Image Generated') }}</p>
                                            <p class="statistics-title text-center d-xl-none">{{ __('Image') }}</p>
                                            <h3 class="rate-percentage text-center">{{ $image_used }} / {{ $image_limit }}</h3>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                                    ...
                                                    <div class="d-sm-flex justify-content-between align-items-start">
                                                        <div>
                                                            <h4 class="card-title card-title-dash">
                                                                {{ __('Text Token Usage') }}</h4>
                                                            <h5 class="card-subtitle card-subtitle-dash">
                                                                {{ __('Last week vs this week') }}</h5>
                                                        </div>
                                                    ...
                                                    <div class="row">
                                                        <div class="col-sm-6">
                                                            <div
                                                                ...
                                                                <div>
                                                                    <p class="text-small mb-2">{{ __('Image generated') }}
                                                                    </p>
                                                                    <h4 class="mb-0 fw-bold">
                                                                        {{ $this_week_token_image_usage_count }}</h4>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="col-sm-6">
                                                            <div class="d-flex justify-content-between align-items-center">
                                                                <div>
                                                                    <p class="text-small mb-2">{{ __('Speech to text') }}
                                                                    </p>
                                                                    <h4 class="mb-0 fw-bold">
                                                                        {{ $this_week_token_audio_usage_count }}
                                                                        {{ __('min') }}</h4>
                                                                </div>
                                                            </div>
                                                        </div>
                                            ...
                                            <div class="card-body">
                                                <div class="col-sm-8">
                                                    ...
                                                    <a href="{{ route('pricing-plan') }}"
                                                        class="btn btn-sm btn-warning text-dark border-0 m-0">{{ __('Upgrade Account') }}</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            @else
                                <?php
                                    $user_package = \App\Models\Package::find(Auth::user()->package_id);
                                ?>
                                      ...
                                      <div class="col-12 grid-margin stretch-card">
                                          <div class="card card-rounded table-darkBGImg">
                                              <div class="card-body">
                                                  <div class="col-sm-8">
                                                      <h5 class="text-white upgrade-info mb-4">
                                                          You have subscribe our {{ $user_package->package_name }} plan and it will be expire on {{date('d M Y',strtotime(Auth::user()->expired_date))}}
                                                      </h5>
                                                      {{-- <a href="{{ route('pricing-plan') }}"
                                                          class="btn btn-sm btn-warning text-dark border-0 m-0">{{ __('Upgrade Account') }}</a> --}}
                                                  </div>
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="row">
                                                <div class="form-group m-0">
                                                    <input type="text" class="fs-6 form-control form-control-xl border-2"
                                                        autofocus onkeyup="search_library_item(this,'library-item')"
                                                        placeholder="{{ __('Just type what you are looking for') }}...">
                                                </div>
                                                <div class="col-md-12 mx-auto">
                                                    <ul class="nav nav-pills nav-pills-custom" id="pills-tab-custom"
                                                        role="tablist">
                                                        <li class="nav-item">
                                                            <a class="nav-link px-3 fw-bold active"
                                                                id="pills-home-tab-custom" data-bs-toggle="pill"
                                                                href="#pills-library-all" role="tab"
                                                                aria-controls="pills-library-all" aria-selected="true">
                                                        ...
                                                        @foreach ($ai_sidebar_group_by_id as $menu_group_id => $menu_items)
                                                            ...
                                                            if (!$has_access) {
                                                                $group_icon = 'fa-solid fa-award text-danger';
                                                            }
                                                            $upgrade_text = !$has_access ? '<span class="badge badge-opacity-danger p-1 ms-1"><small>' . __('Pro') . '</small></span>' : '';
                                                            ?>
                                                            <li class="nav-item">
                                                                <a class="nav-link px-3 fw-bold"
                                                                    id="pills-home-tab-custom" data-bs-toggle="pill"
                                                                    href="#pills-{{ $tab_library_id }}" role="tab"
                                                                    aria-controls="pills-{{ $tab_library_id }}"
                                                                    aria-selected="true">
                                                                    <i class="{{ $group_icon }}"></i>
                                                                    {{ $group_name }} {!! $upgrade_text !!}
                                                                </a>
                                                            </li>
                                                        @endforeach
                                                    </ul>
                                ...
                                <div class="col-12">
                                    <div class="tab-content tab-content-custom-pill" id="pills-tabContent-custom">
                                        ...
                                        @foreach ($ai_sidebar_group_by_id as $menu_group_id => $menu_items)
                                            ...
                                            $tab_library_id = 'tab-library-' . $tab_number;
                                            ?>
                                            <div class="tab-pane fade" id="pills-{{ $tab_library_id }}" role="tabpanel"
                                                ...
                                                <div class="row">
                                                    @foreach ($menu_items as $menu_item)
                                                        ...
                                                        $has_access = $menu_items[0]['has_access'] ?? true;
                                                        if (!$has_access) {
                                                            $menu_item['template_thumb'] = 'fa-solid fa-award text-danger';
                                                        }
                                                        $upgrade_text = !$has_access ? '<span class="badge badge-opacity-danger p-1 ms-1"><small>' . __('Pro') . '</small></span>' : '';
                                                        ?>
                                                        <div
                                                            class="col-12 col-sm-6 col-md-6 col-lg-6 col-xl-4 grid-margin library-item">
                                                            <div class="card d-flex align-items-start cursor-pointer"
                                                                onclick="return window.location.href='<?php echo $action_url; ?>'">
                                                                <div class="card-body">
                                                                    <div class="d-flex flex-row align-items-start">
                                                                        <i
                                                                            class="{{ $menu_item['template_thumb'] }} icon-lg"></i>
                                                                        <div class="ms-3">
                                                                            <h6 class="text-twitter mb-0">
                                                                                {{ __($menu_item['template_name']) }}
                                                                                {!! $upgrade_text !!}</h6>
                                                                            <p class="mt-2 text-dark card-text">
                                                                                {{ __($menu_item['about_text']) }}</p>
                                                                            <p class="text-muted">
                                                                                {{ __($menu_item['template_description']) }}
                                                                            </p>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                        ...
                                        @endforeach
                                        <div class="tab-pane fade show active" id="pills-library-all" role="tabpanel"
                                            aria-labelledby="pills-home-tab-custom">
                                            <div class="row">
                                                        ...
                                                        <div class="card d-flex align-items-start cursor-pointer"
                                                            ...
                                                            <div class="card-body">
                                                                <div class="d-flex flex-row align-items-start">
                                                                    ...
                                                                    <div class="ms-3">
                                                                        <h6 class="text-twitter mb-0">
                                                                            {{ __($menu_item['template_name']) }}</h6>
                                                                        <p class="mt-2 text-dark card-text">
                                                                            {{ __($menu_item['about_text']) }}
                                                                            {!! $upgrade_text !!}</p>
                                                                        <p class="text-muted">
                                                                            {{ __($menu_item['template_description']) }}
                                                                        </p>
