...
<html lang="{{ get_current_lang() }}" class="<?php if ($enable_dark_mode == '1') {
    echo 'dark';
} ?>">

<head>
<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-JV1CTQ9SXC"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'G-JV1CTQ9SXC');
</script>
    <meta charset="UTF-8" />
    ...
    <header class="header absolute top-0 left-0 w-full">
        <div class="flex w-full flex-wrap px-5 lg:flex-nowrap lg:items-center lg:px-5 xl:px-10 2xl:px-20">
            ...
            <div
                        ...
                        <ul
                            ...
                            @if ($disable_landing_page == '0')
                                ...
                                @if (config('app.is_demo') == '1')
                                    <li>
                                        <a href="{{ $get_landing_language->links_docs_url ?? route('docs') }}"
                                            class="menu-scroll inline-flex items-center justify-center text-center font-heading text-base text-dark-text hover:text-primary dark:hover:text-white">
                                            {{ __('Documentation') }} </a>
                                    </li>
                                @endif
                                <li class="mobileshow">
                                    <a href="{{ route('login') }}"
                                        class="w-full whitespace-nowrap rounded bg-primary py-3 px-6 text-center font-heading text-white hover:bg-opacity-90 lg:w-auto" style="max-height: 45px; ">
                                        <?php if (Auth::user()) {
                                            echo __('Dashboard');
                                        } else {
                                            echo __('Sign In');
                ...
                <div
                    class="hidden-mobile absolute bottom-0 left-0 flex w-full items-center justify-between space-x-5 self-end p-5 lg:static lg:w-auto lg:self-center lg:p-0">
                    <a href="{{ route('login') }}"
                        class="w-full whitespace-nowrap rounded bg-primary py-3 px-6 text-center font-heading text-white hover:bg-opacity-90 lg:w-auto" style="max-height: 45px; ">
                        <?php if (Auth::user()) {
                            echo __('Dashboard');
            ...
            <div class="-mx-4 flex flex-wrap">
                <div class="w-full px-4 sm:w-1/2 md:w-5/12 lg:w-5/12 xl:w-5/12">
                    <div class="mb-20 max-w-[330px]">
                        <a href="{{ route('home') }}" class="mb-6 inline-block">
                            <img src="{{ config('app.logo_alt') }}" alt="logo"
                                class="hidden h-[50px] dark:block" />
                            <img src="{{ config('app.logo') }}" alt="logo" class="h-[50px] dark:hidden" />
                        </a>
                        <p class="mb-10 text-base text-dark-text">
                            {{ __('Produce content for your blogs, advertisements, emails, and website that is both search engine optimized and free of plagiarism, all while increasing your speed by a factor of 10.') }}
                        </p>
                        <div class="flex items-center space-x-5">
                            @if (isset($get_landing_language->company_telegram_channel) && !empty($get_landing_language->company_telegram_channel))
                                <a href="{{ $get_landing_language->company_telegram_channel }}" target="_BLANK"
                            ...
                            @if (isset($get_landing_language->company_fb_page) && !empty($get_landing_language->company_fb_page))
                                <a href="{{ $get_landing_language->company_fb_page }}" target="_BLANK"
                                    name="social-link" aria-label="social-link"
                                    class="text-dark-text hover:text-primary dark:hover:text-white">
                            ...
                            @if (isset($get_landing_language->company_youtube_channel) && !empty($get_landing_language->company_youtube_channel))
                                <a href="{{ $get_landing_language->company_youtube_channel }}" target="_BLANK"
                                    name="social-link" aria-label="social-link"
                                    class="text-dark-text hover:text-primary dark:hover:text-white">
                            ...
                            @if (isset($get_landing_language->company_twitter_account) && !empty($get_landing_language->company_twitter_account))
                                <a href="{{ $get_landing_language->company_twitter_account }}" target="_BLANK"
                                    name="social-link" aria-label="social-link"
                                    class="text-dark-text hover:text-primary dark:hover:text-white">
                            ...
                            @if (isset($get_landing_language->company_instagram_account) && !empty($get_landing_language->company_instagram_account))
                                <a href="{{ $get_landing_language->company_instagram_account }}" target="_BLANK"
                                    name="social-link" aria-label="social-link"
                                    class="text-dark-text hover:text-primary dark:hover:text-white">
                            ...
                            @if (isset($get_landing_language->company_linkedin_channel) && !empty($get_landing_language->company_linkedin_channel))
                                <a href="{{ $get_landing_language->company_linkedin_channel }}" target="_BLANK"
                                    name="social-link" aria-label="social-link"
                                    class="text-dark-text hover:text-primary dark:hover:text-white">
                ...
                <div class="w-1/2 px-4 md:w-3/12 lg:w-3/12 xl:w-2/12">
                    <div class="mb-20">
                        ...
                        <ul class="space-y-4">
                            <li>
                                <a href="{{ route('home') }}"
                                    class="font-heading text-base text-dark-text hover:text-primary dark:hover:text-white">{{ __('Home') }}</a>
                            </li>
                            <li>
                                <a href="{{ route('register') }}"
                                    class="font-heading text-base text-dark-text hover:text-primary dark:hover:text-white">{{ __('Sign Up') }}</a>
                            </li>
                            <li>
                                <a href="{{ route('pricing-plan') }}"
                                    class="font-heading text-base text-dark-text hover:text-primary dark:hover:text-white">{{ __('Pricing') }}</a>
                            </li>
                            <?php $company_support_url = $get_landing_language->company_support_url ?? ''; ?>
                            @if (!empty($company_support_url))
                                <li>
                                    <a href="{{ $company_support_url }}"
                                        class="font-heading text-base text-dark-text hover:text-primary dark:hover:text-white">{{ __('Support') }}</a>
                                </li>
                            @endif
                        </ul>
                    </div>
                ...
                <div class="w-1/2 px-4 md:w-3/12 lg:w-3/12 xl:w-2/12">
                    <div class="mb-20">
                        ...
                        <ul class="space-y-4">
                            <?php
                                $static_pages = \App\Models\Policy::get();    
                            ?>
                            @foreach ($static_pages as $static_page)
                                <li>
                                    <a href="{{ route('policy-static',$static_page->url) }}"
                                        class="font-heading text-base text-dark-text hover:text-primary dark:hover:text-white">{{ $static_page->name }}</a>
                                </li>
                            @endforeach
                        </ul>
                    </div>
            ...
            <div class="dark:border-[#2E333D] md:border-t">
                <div class="-mx-4 flex flex-wrap py-5 md:py-7">
                    ...
                    <div class="w-full px-4">
                        <div>
                            <p class="text-center font-heading text-base text-dark-text"> © {{ date('Y') }}
                                {{ config('app.name') }}. {{ __('All rights reserved') }} <br /><span>A unit of
                                    Shibani Web Store Pvt. Ltd.</span></p>
                        </div>
                    </div>
