...
$has_settings_access = has_module_access($module_id_settings, $user_module_ids, $is_manager);
$has_template_manager_access = has_module_access($module_id_template_manager, $user_module_ids, $is_manager);
...
<nav class="sidebar sidebar-offcanvas" id="sidebar">
    <ul class="nav">
        ...
        @if (!empty($ai_sidebar_group_by_id))
            ...
            @if (has_module_access($ai_livechat, $user_module_ids, $is_admin))
                <?php
            ...
            @foreach ($ai_sidebar_group_by_id as $menu_group_id => $menu_items)
                ...
                ?>
                <li class="nav-item">
                    <a class="nav-link" data-bs-toggle="collapse" href="#{{ $ai_sidebar_menu_group_id }}"
                        aria-expanded="false" aria-controls="{{ $ai_sidebar_menu_group_id }}">
                        ...
                        <ul class="nav flex-column sub-menu">
                            @foreach ($menu_items as $menu_item)
                                <li class="nav-item"> <a class="nav-link"
                                        href="{{ route('tools', ['group_slug' => $menu_item['group_slug'] ?? '', 'template_slug' => $menu_item['template_slug'] ?? '']) }}">{{ __($menu_item['template_name']) }}</a>
                                </li>
                            @endforeach
                        </ul>
                    </div>
            ...
            @endforeach
            <li class="nav-item">
                <a class="nav-link" data-bs-toggle="collapse" href="#image-generation" aria-expanded="false" aria-controls="image-generation">
                    <i class="mdi mdi-image-edit text-primary menu-icon"></i>
                    <span class="menu-title">Image Generator </span>
                </a>
                <div class="collapse" id="image-generation">
                    <ul class="nav flex-column sub-menu">
                        <li class="nav-item">
                            <a class="nav-link" href="{{ route('image-generator.view') }}">Image Generator</a>
                        </li>
                    </ul>
                </div>
            </li>
        ...
        @if ($route_name != 'livechat.load')
            @if ($is_admin)
                <li class="nav-item nav-category">{{ $is_admin ? __('Administration') : __('Management') }}</li>
            @endif
            <?php
              $thisRoute = 'template-manager';
              $get_selected_sidebar_class = $get_selected_sidebar == $thisRoute ? 'active' : '';
            ?>
            @if ($is_admin || $has_template_manager_access)
                <li class="nav-item {{ $get_selected_sidebar_class }}">
                    <a class="nav-link {{ $get_selected_sidebar_class }}" href="{{ route($thisRoute) }}">
            ...
            @if ($is_admin)
                <li class="nav-item {{ $get_selected_sidebar_class }}">
                    <a class="nav-link {{ $get_selected_sidebar_class }}" href="{{ route($thisRoute) }}">
                        <i class="mdi mdi-google-play menu-icon"></i>
