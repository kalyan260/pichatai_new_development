...
<head>
    ...
    @if (isset($load_datatable) && $load_datatable)
        ...
        <link rel="stylesheet"
            href="{{ asset('assets/vendors/datatables/DataTables-1.10.25/css/dataTables.bootstrap5.min.css') }}">
        <link rel="stylesheet"
            href="{{ asset('assets/vendors/datatables/ColReorder-1.5.4/css/colReorder.bootstrap5.min.css') }}">
        <link rel="stylesheet"
            href="{{ asset('assets/vendors/datatables/Buttons-1.7.1/css/buttons.bootstrap5.min.css') }}">
...
<body class="with-welcome-text <?php if (in_array($route_name, $full_width_page_routes)) {
    echo ' sidebar-icon-only ';
} ?> <?php if (isset($load_box_layout) && $load_box_layout) {
    echo 'boxed-layout';
} ?> {{ config('app.localeDirection') }}">
