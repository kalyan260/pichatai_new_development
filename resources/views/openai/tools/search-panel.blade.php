...
@section('content')
    <div class="content-wrapper pt-1">
        <div class="row d-none d-xl-flex">
            @if (isset($ai_sidebar_group_by_id[$template_group_data->id]) &&
                    count($ai_sidebar_group_by_id[$template_group_data->id]) > 1)
                ...
                ?>
                <div class="col-xl-3 ps-xl-0 mb-3 grid-margin stretch-card cursor-pointer"
                    onclick="return window.location.href='<?php echo $action_url; ?>'">
                    <div class="card bg-primary text-white border">
                        <div class="card-body p-0">
                            <div class="d-sm-flex flex-row flex-wrap text-start align-items-center">
                                <div class="py-2 text-center w-80px"><i class="icon-xl text-white {{ $template_thumb }}"></i>
                                </div>
                ...
                @foreach ($ai_sidebar_group_by_id[$template_group_data->id] as $related_item)
                    <?php
                    if ($related_item['id'] == $template_data->id) {
                        continue;
                    }
                    ...
                    ?>
                    <div class="col-xl-3 <?php if ($related_count % 3 == 0) {
                        echo ' pe-xl-0 ';
                    } ?> ps-xl-0 mb-3 grid-margin stretch-card cursor-pointer"
                        onclick="return window.location.href='<?php echo $action_url; ?>'">
                        <div class="card">
                            <div class="card-body p-0">
                                <div class="d-sm-flex flex-row flex-wrap text-start align-items-center">
                ...
                <div class="col-lg-4 pt-4 pt-md-2 pb-4 bg-white">
                    <div class="menu-bar p-0 p-xl-2">
                        <form method="POST" action="" class="mt-2" id="generate-form">
                            ...
                            <input type="hidden" id="group-slug"
                                value="{{ htmlspecialchars($template_group_data->group_slug ?? '') }}">
                            <input type="hidden" id="template-slug"
                                value="{{ htmlspecialchars($template_data->template_slug ?? '') }}">
                            <div class="col-12">
                                <div class="form-group">
                                    <label><?php echo __('Document Title'); ?></label>
                                    <input type="text" name="document_name" autofocus placeholder="Untitled Document"
                                        id="document_name" value="{{ $search_data->document_name ?? '' }}"
                        ...
                        ?>
                                ...
                                @foreach ($prompt_fields as $prompt_key => $prompt_val)
                                    <div class="col-12">
                                        <div class="form-group repeater">
                                            ...
                                            <div class="input-group mb-2 mr-sm-2 mb-sm-0">
                                                <input type="hidden" value="{{ $prompt_key }}" name="paramName[]"
                                                    class="form-control form-control-lg paramName">
                                                ...
                                                @elseif($prompt_val == 'dropdown')
                                                    <select name="paramValue[]" required
                                                        class="form-control form-control-lg paramValue">
                                                        <option class="form-group" value="">
                                                            {{ __('Select ') . '  ' . $prompt_key }}</option>
                                                        @foreach ($paramType_drop_down_values[$prompt_key] as $dropdown_value)
                                                            <option class="form-group"
                                                                @if (isset($prompt_fields_data[$prompt_key]) && $prompt_fields_data[$prompt_key] == $dropdown_value) selected @endif
                                                                value="{{ $dropdown_value }}">{{ $dropdown_value }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                @else
                                                    <input type="{{ $prompt_val }}"
                                                        value="{{ $prompt_fields_data[$prompt_key] ?? '' }}" required
                                                        name="paramValue[]" class="form-control form-control-lg paramValue">
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                            ...
                            @endif
                            @if (
                                ($template_data->api_group == 'image' && $template_data->api_type != 'images/generations') ||
                                    $template_data->api_group == 'audio')
                                <div class="col-12">
                                    <div class="form-group">
                                        <?php $upload_details = $template_data->api_group == 'image' ? 'png, <4MB, ' . __('Square') : '.mp3,.mp4,.mpeg,.mpga,.m4a,.wav,.webm'; ?>
                                        <label><?php echo __('Upload Media'); ?>* <span
                                                class="text-small text-muted">({{ $upload_details }})</span></label>
                                        ...
                                        <div id="media-url-dropzone" class="dropzone mb-1">
                                            <div class="dz-default dz-message">
                                                <input class="form-control form-control-lg" name="media_url_dropzone"
                                                    id="media-uploaded-file" type="hidden">
                            ...
                            @if ($template_data->api_group != 'image' && $template_data->api_group != 'audio')
                                <div class="row">
                                    <div class="col-12 col-xl-6  pe-xl-1">
                                        <div class="form-group">
                                            <label><?php echo __('Language'); ?> <i class="fas fa-info-circle text-dark"
                                                    data-bs-toggle="tooltip"
                                                    title="{{ __('Specify the desired language for generating your content.') }}"></i></label>
                                            <?php
                                    ...
                                    <div class="col-12 col-xl-6 ps-xl-1">
                                        <div class="form-group">
                                            <label><?php echo __('Creativity Level'); ?> <i class="fas fa-info-circle text-dark"
                                                    data-bs-toggle="tooltip"
                                                    title="{{ __('Creativity refers to the degree of novelty and uniqueness in the generated text. Higher levels of creativity can lead to more original and unexpected results, while lower levels may result in more predictable and conservative text.') }}"></i></label>
                            ...
                            @if ($template_data->api_group == 'text' || $template_data->api_group == 'chat')
                                <div class="row">
                                    {{-- <a class="btn btn-lg btn-primary fw-bol btn-block mt-4 mb-4 w-100" id="adbance">advance settings</a> --}}
                                    {{-- <a class="btn btn-lg btn-primary fw-bol btn-block mt-4 mb-4 w-100" id="adbance">advance settings</a> --}}
                                    <p style="color: #ca0acf; font-size: 15px; cursor: pointer;" id="adbance-show">
                                        <i class="fas fa-arrow-right" style="padding-right: 10px;"></i>
                                        Advance Settings
                                    </p>
                                    <p style="color: #ca0acf; font-size: 15px; display: none; cursor: pointer;"
                                        id="adbance-hide">
                                        <i class="fas fa-arrow-down" style="padding-right: 10px;"></i>
                                        Advance Settings
                                    </p>
                                </div>
                                <div class="row advance-block" id="" style="display:none;">
                                    <div class="col-12 col-xl-6 pe-xl-1">
                                        <div class="form-group">
                                            <label><?php echo __('Phrase Repetition Level'); ?> <i class="fas fa-info-circle text-dark"
                                                    data-bs-toggle="tooltip"
                                                    title="{{ __('Phrase repetition level refers to the frequency of repeating phrases in the generated text. Setting a high penalty for phrase repetition can reduce the occurrence of repetitive phrases in the output, while setting a lower penalty can result in more repetitions.') }}"></i></label>
                                    ...
                                    <div class="col-12 col-xl-6 ps-xl-1">
                                        <div class="form-group">
                                            <label><?php echo __('Input Presence Level'); ?> <i class="fas fa-info-circle text-dark"
                                                    data-bs-toggle="tooltip"
                                                    title="{{ __('Input presence level refers to how much the generated text is influenced by the input text provided. A high input presence level ensures that the generated text is closely related to the input, while a lower input presence level allows for more divergence from the input text.') }}"></i></label>
                            ...
                            <div class="row">
                                ...
                                @endif
                                @if ($template_data->api_group == 'text' || $template_data->api_group == 'chat')
                                    <div class="col-12 col-xl-8 pe-xl-1" style="display: none">
                                        <div class="form-group">
                                            <label><?php echo __('Max Tokens'); ?> <i class="fas fa-info-circle text-dark"
                                                    data-bs-toggle="tooltip"
                                                    title="{{ __('You can think of tokens as pieces of words used for natural language processing. For English text, 1 token is approximately 4 characters or 0.75 words. As a point of reference, the collected works of Shakespeare are about 900,000 words or 1.2M tokens.') }}"></i></label>
                                            <input type="number" value="500" min="16" max="32768"
                                                name="max_tokens" id="max_tokens" class="form-control form-control-lg">
                                ...
                                if ($template_data->api_group == 'audio') {
                                    $variation_class = '';
                                }
                                ?>
                                <div class="col-12 {{ $variation_class }}" style="display: none">
                                    <div class="form-group">
                                        <label><?php echo __('Variation'); ?> <i class="fas fa-info-circle text-dark"
                                                data-bs-toggle="tooltip"
                                                title="{{ __('It determines the number of completions to generate for each submit. However, it is important to note that this can consume a significant amount of your token quota due to the high number of variations it generates. Therefore, use it carefully and make sure to have appropriate settings for max tokens and stop.') }}"></i></label>
                                        <input type="number" value="4" min="1" max="10"
                                            name="variation" id="variation" class="form-control form-control-lg">
                                        <span id="variation_err" class="text-danger"></span>
                                    </div>
                                </div>
                            </div>
                            <button class="btn btn-lg btn-primary fw-bol btn-block mt-2 w-100" id="generate"
                                type="submit"><i class="fas fa-pen-nib"></i>&nbsp;&nbsp;{{ __('Generate') }}</button>
                        </form>
                    </div>
                </div>
                                ...
                                @foreach ($choices as $choice)
                                    <form action="{{ route('tools-download-text') }}" method="post">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div
                                                    class="btn-toolbar pt-3 px-3 px-lg-4 pt-4 pb-0 d-flex justify-content-between">
                                                    <div class="btn-group">
                                                        <button type="button"
                                                            class="btn btn-sm fw-bolder px-0">{{ $choice_count > 0 ? __('Variation') . '-' . $choice_count : __('Output') }}</button>
                                                    </div>
                                                    <div class="btn-group">
                                                        <button type="button"
                                                            onclick="copyToClipboard('choice-{{ $choice_count }}')"
                                                            class="btn btn-sm px-0"><i
                                                                class="ti-layers text-primary me-1"></i>{{ __('Copy') }}</button>
                                                        <input type="hidden" name="filename"
                                                            value="{{ $template_data->template_slug . '.' . $template_group_data->group_slug . '.search-' . $search_data->id . '.output-' . $choice_count + 1 }}">
                                                        <button type="submit" class="btn btn-sm px-0 ps-4"><i
                                                                class="ti-download text-primary me-1"></i>{{ __('Download') }}</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="form-group px-3 px-lg-4 pb-0">
                                            @if ($template_data->output_display == 'code')
                                                <textarea name="download_text" id="choice-{{ $choice_count }}" onblur="auto_grow(this)" class="search-result d-none">
                                                ...
                                                <textarea name="" id="editor-{{ $choice_count }}" class="search-result code-editor">
                                                    @if (isset($choice->text))
                                                        {!! $choice->text !!}
                                                    @else
                                                        @if (isset($choice->message) && isset($choice->message->content))
                                            ...
                                            @else
                                                <textarea name="download_text" id="choice-{{ $choice_count }}" onblur="auto_grow(this)"
                                                    class="search-result {{ $template_data->output_display == 'code' ? ' code-editor' : '' }}">{!! $choice->text !!}</textarea>
                                            @endif
                                        </div>
                                    </form>
                                    <?php $choice_count++; ?>
                                    ...
                                    <form action="{{ route('tools-download-file') }}" method="post">
                                        ...
                                        @if (@getimagesize($choice->url))
                                            <div class="mx-3 mb-5 mx-lg-4">
                                                <img src="{{ $choice->url }}"
                                                    alt="{{ __('Resource has been moved or removed.') }}"
                                        ...
                                        @else
                                            <div class="mx-3 my-3 mx-lg-4">
                                                <div class="alert alert-warning">
                                                    <h4 class="mt-2">
                                                        {{ __("The resource's validity has expired and it has been removed. You can re-generate content using same dataset.") }}
                                                    </h4>
                                                </div>
                                            </div>
                                        @endif
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div
                                                ...
                                                <div class="btn-group">
                                                    <button type="button"
                                                        class="btn btn-sm fw-bolder px-0">{{ __('Output') }}</button>
                                                </div>
                                                <div class="btn-group">
                                                    <button type="button" onclick="copyToClipboard('choice')"
                                                        class="btn btn-sm px-0"><i
                                                            class="ti-layers text-primary me-1"></i>{{ __('Copy') }}</button>
                                                    <input type="hidden" name="filename"
                                                        value="{{ $template_data->template_slug . '.' . $template_group_data->group_slug . '.search-' . $search_data->id . '.output' }}">
                                                    <button type="submit" class="btn btn-sm px-0 ps-4"><i
                                                            class="ti-download text-primary me-1"></i>{{ __('Download') }}</button>
                                                </div>
                                            </div>
                    ...
                    @elseif(isset($response->error))
                        <div class="row">
                            <div class="col-12 text-center">
                                <img src="{{ asset('assets/images/content-error.jpg') }}" alt=""
                                    class="mt-5 pt-5 content-error">
                                <h4 class="my-4 pb-5 text-danger px-4">
                                    {{ $response->error->message ?? __('Something went wrong.') }}</h4>
                            </div>
                        </div>
                    @else
                        <div class="row d-none d-md-flex">
                            <div class="col-12 text-center">
                                <img src="{{ asset('assets/images/content-pending.jpg') }}" alt=""
                                    class="mt-5 pt-5 content-pending">
                                <h4 class="my-4 pb-5 text-primary px-4">
                                    {{ __('When done, your desired content will be displayed here.') }}</h4>
                            </div>
                        </div>
                    @endif
                </div>
...
@push('scripts-footer')
    <script>
        $(document).ready(function() {
            $("#adbance-show").click(function() {
                $(".advance-block").show();
                $("#adbance-show").hide();
                $("#adbance-hide").show();
            });
            $("#adbance-hide").click(function() {
                $(".advance-block").hide();
                $("#adbance-hide").hide();
                $("#adbance-show").show();
            });
