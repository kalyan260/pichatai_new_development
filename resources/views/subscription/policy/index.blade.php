<?php
$lang_display = __('Policy');
$title_display = __('Policy');
?>
            ...
            <div class="card-body card-rounded">
                <div class="d-sm-flex justify-content-between align-items-start">
                    <div>
                        ...
                        <p class="card-subtitle card-subtitle-dash mb-2 mb-lg-4">{{ __('Manage Policy') }}</p>
                    </div>
                    <div class="btn-wrapper mb-2">
                        <div class="btn-group" role="group" aria-label="Button group with nested dropdown d-inline">
                            <div class="btn-group" role="group">
                                <a href="{{ route('policy.add') }}" type="button" class="btn btn-otline-dark btn-sm makeDefault">
                                     {{ __('Create') }}
                                </a>
                            </div>
                        </div>
                            ...
                            @foreach ($items as $item)
                                ...
                                <tr>
                                    ...
                                    <td class=" text-center">
                                        <a class="btn btn-info btn-sm" href="{{ route('policy.edit',$item->id) }}">
                                            <i class="fas fa-pencil-alt">
                                            </i>
                                            Edit
                                        </a>
                                        <a onclick="onClickDelete(event)" class="btn btn-danger btn-sm" href="{{ route('policy.delete',$item->id) }}">
                                            <i class="fas fa-user">
                                            </i>
                                            Delete
                                        </a>
