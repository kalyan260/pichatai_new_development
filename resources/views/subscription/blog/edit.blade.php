<?php
    ...
    $title_display = __('Update Blog');
    $title_display_des = __('Update Blog');
...
@section('content')
    <div class="content-wrapper">
        <form class="form form-vertical" enctype="multipart/form-data" method="POST" action="{{ route('blog.edit',$item->id) }}">
            @csrf
            <div class="row">
                <div class='col-12 col-md-12'>
                    <div class="card card-rounded mb-4 p-lg-2">
                        <div class="card-body card-rounded">
                            <h4 class="card-title card-title-dash mb-4">{{ __('Update A Blog') }}</h4>
                            <div class="row">
                                <div class="col-6">
                                    <div class="form-group">
                                        <label for="name">Title*</label>
                                        <input name="name" class="form-control form-control-lg form-control form-control-lg-lg" type="text" value="{{ $item->name }}">
                                    </div>
                                </div>
                                <div class="col-6">
                                    <div class="form-group">
                                        <label for="name">Image</label>
                                        <input name="photo" class="form-control form-control-lg form-control form-control-lg-lg" type="file">
                                    </div>
                                </div>
                                <div class="col-12">
                                    <div class="form-group">
                                        <label for="name">Description*</label>
                                        <textarea class="summernote" name="description" required>{!! $item->description !!}</textarea>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
